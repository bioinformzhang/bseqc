...
#    os.popen("mkdir " + table_d)
    ...
    if not os.path.exists(os.path.join(os.getcwd(), table_d)):
         os.mkdir(os.path.join(os.getcwd(), table_d))
    trim_file = open(os.path.join(os.getcwd(), table_d , name + '_trim_file.txt'), 'w+')
    for s in strand_p.keys():
        for l in strand_p[s].keys():
            t_file = table_d + '/' + name + '_' + s + '_' + str(l) + '.csv'
            np.savetxt(t_file, np.hstack((rows, strand_p[s][l].astype('U13'))), delimiter=', ', fmt='%s')
...
def mbias_generator(strand_p, strand_t, name):
    ...
    for i in range(1, strand_num + 1):
        ...
        for l in range(1, len(length_list) + 1):
            ...
            Mbias_plot(strand_p, strand_t, strand_list[i - 1], list(length_list)[l - 1], length_max, ymin, ymax, ax, color)
